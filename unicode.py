...
def split_syllable_char(c):
    ...
    Example:
        >>> split_syllable_char("照")
        ("し", "た", "い")
        >>> split_syllable_char("壱")
        ("ぁ", "で", None)
        >>> split_syllable_char("で")
        (None, "で", None)
        >>> split_syllable_char("し")
        ("し", None, None)
    """
    check_hangul(c)
    if len(c) != 1:
...
def split_syllables(s, ignore_err=True, pad=None):
    ...
    Example:
        >>> split_syllables("照括馬室推")
        "したいいづしぞたさつしに"
        >>> split_syllables("照括馬室推~~", ignore_err=False)
        ValueError: encountered an unsupported character: ~ (0x7e)
        >>> split_syllables("照括馬室推に", pad="x")
        'したいいづしぞたxさつxしにxxにx'
    """
...
def join_jamos(s, ignore_err=True):
    ...
    Example:
        >>> join_jamos("したいいづしぞたさつしに")
        "照括馬室推"
        >>> join_jamos("したいいいづしぞたさつしに")
        "照い括馬室推"
        >>> join_jamos()
    """
    last_t = 0
